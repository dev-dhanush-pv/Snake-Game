...
let startGame = ()=>{
    ...
    food.style.left = '140px'
    food.style.top = '100px'
    area.append(head,body,food)
}
startGame()

let head = document.querySelector('#head');

// Reset the variables after game ended
let gameOver = ()=>{
    clearInterval(begin);
    area.removeChild(document.getElementById('head'))
    ...
    left = 0
    up = 0
    area.innerHTML="<p>GAME OVER</p>"
}

// Move the snake by a single grid
let move = (a,b)=>{
...
// Growing the snake's body after it has eaten
let grow = ()=>{
...
let valid_move = ()=>{
    ...
    let snake = [...document.getElementsByClassName('body')]
    snake = snake.map((x)=>{    return [x.style.top.split('px'), x.style.left.split('px')]  })

    let snake_touch_itself = snake.every( (x)=>{
            return Number(x[0][0]) != snake_body_obj[direction][0] || Number(x[1][0]) != snake_body_obj[direction][1]
        })
    let snake_touch_walls = 0<=snake_body_obj[direction][0] && snake_body_obj[direction][0]<=480 && 0<=snake_body_obj[direction][1] && 980>=snake_body_obj[direction][1]

    return (snake_touch_itself && snake_touch_walls)
}
